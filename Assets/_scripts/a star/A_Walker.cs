...
public class A_Walker : MonoBehaviour {

    public float mSpeed = 2f;

	public void WalkPath(A_Waypoint goal)
    {
        StartCoroutine(FollowWaypoints(goal));
    }
    ...
    IEnumerator FollowWaypoints(A_Waypoint goal)
    {
        ...
        {
            transform.position += (nodeWorldPos - transform.position).normalized * Time.deltaTime * mSpeed;
